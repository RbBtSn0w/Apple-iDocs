...
- **Source Checkout CLI Usage**: When validating commands from this checkout, replace installed `idocs` calls with `./scripts/tuist-silent.sh run idocs -- ...`. Agent and benchmark workflows can isolate state with `IDOCS_CACHE_PATH` and `IDOCS_USAGE_LOG_PATH`.
