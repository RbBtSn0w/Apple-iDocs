...
- **FR-003**: **Common Layer (iDocsKit)** MUST expose configuration and logger injection points to allow Application layers to pass environment-specific settings. It MUST define an explicit `DocumentationConfig` type containing, at minimum: cache path, API endpoint base URL, locale, timeout settings, and any flags needed for optional features (e.g., file locking for shared-cache setups).
...
- **FR-008**: **Common Layer (iDocsKit)** SHOULD implement file-based locking only if a shared cache directory is introduced across processes, and it MUST be controllable via `DocumentationConfig` (opt-in).
