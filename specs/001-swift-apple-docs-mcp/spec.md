...
4. **Given** 首次在线搜索完成, **When** 相同关键词再次搜索, **Then** 服务器从本地缓存返回结果，速度显著更快
...
- **FR-005**: 系统 MUST 提供本地文档索引的毫秒级符号定位能力
- **FR-006**: 系统 MUST 支持本地 Spotlight 搜索用于全量模糊匹配
- **FR-007**: 系统 MUST 提供 7 个精简的 MCP 工具，每个工具独立可用、无状态
- **FR-008**: 系统 MUST 实现磁盘持久化缓存，支持 TTL 过期管理
- **FR-009**: 系统 MUST 支持内存级 LRU 缓存以加速高频查询
...
- **FR-015**: 系统 MUST 在 API 返回 403/429 时自动切换请求标识重试
- **FR-016**: 系统 MUST 在本地文档不可用时静默回落到在线 API，不向用户报错
...
- **FR-022**: 系统 MUST 对大文件使用内存映射技术以降低系统资源消耗
