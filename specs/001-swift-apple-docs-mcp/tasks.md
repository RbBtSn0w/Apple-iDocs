...
- [x] T019 [P] [US2] 编写 `DocCRenderer` 单元测试（各节点类型渲染、递归深度保护、边界情况、golden file 对比验证 SC-003 语义结构 100% 保留）in `Tests/iDocsTests/RenderingTests.swift`

### 实现

- [x] T020 [US2] 实现 `DocCRenderer`（DocC JSON → Markdown，覆盖 declarations/parameters/properties/content/tables/aside/codeListing/images/relationship/topic/seeAlso，递归深度保护 50 层 content + 20 层 inline）in `Sources/iDocs/Rendering/DocCRenderer.swift`
- [x] T021 [US2] 实现大文档截断策略（FR-024：体积 >100KB 时按优先级截断代码清单/示例/参考，保留核心声明与参数）in `Sources/iDocs/Rendering/DocCRenderer.swift`
- [x] T022 [US2] 实现 `FetchDocTool` MCP 工具（路径解析，本地缓存优先 → 在线获取，调用 DocCRenderer 渲染，缓存结果）in `Sources/iDocs/Tools/FetchDocTool.swift`
- [x] T023 [US2] 在 `iDocsServer` 中注册 `FetchDocTool` in `Sources/iDocs/iDocsServer.swift`
